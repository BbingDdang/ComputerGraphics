...
void glm_mat_test() {
	...
	A[0][0] = 1.0f;	A[1][0] = 2.0f; A[2][0] = -1.0f; A[3][0] = -1.0f;
	A[0][1] = 2.0f;	A[1][1] = 1.0f; A[2][1] = 0.0f; A[3][1] = 2.0f;
	A[0][2] = 3.0f;	A[1][2] = -2.0f; A[2][2] = 1.0f; A[3][2] = 4.0f;
...
void glm_transform_test() {
	...
	mat_Rotate = glm::rotate(glm::mat4(1.0f), angle, axis);

	//scale
	glm::vec3 scaleFactor(2.0f, 1.0f, 1.5f);
	mat_Scale = glm::scale(glm::mat4(1.0f), scaleFactor);

	//lookat
	glm::vec3 cameraPos(0.0f, 0.0f, -5.0f);
	glm::vec3 targetPos(0.0f, 0.0f, 0.0f);
	glm::vec3 cameraUp(0.0f, 1.0f, 0.0f);
	mat_LookAt = glm::lookAt(cameraPos, targetPos, cameraUp);
	...
	float zNear = 1.0f;
	float zFar = -1.0f;
	mat_Ortho = glm::ortho(left, right, bottom, top, zNear, zFar);
	...
	mat_Frustum = glm::frustum(left, right, bottom, top, nearVal, farVal);
	...
	mat_Perspective = glm::perspective(fieldOfView, aspectRatio, nearClip, farClip);
